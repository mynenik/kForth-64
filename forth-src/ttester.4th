...
\
\ ================ Mods for kForth version ======================== 
\ -- Version 1.16d
\
\ -- Removed environment queries and hardcoded the constants
\      HAS-FLOATING and HAS-FLOATING-STACK
\
\ -- Modified TESTING since kForth does not have >IN
\
\ -- The word T{ is marked as a NONDEFERRED word in kForth,
\      to ensure correct behavior with use of immediate words
\      in a test line (examples in core.4th).
\
\ -- conditional definition of F2DROP and F2DUP
\
\ -- Error messages appear in red; COMMENT added for comment
\      lines, which appear in blue.
\
...
BASE @
DECIMAL
: NORMAL-TEXT ( -- ) 27 EMIT [CHAR] [ EMIT [CHAR] 0 EMIT [CHAR] m EMIT ; 
: RED-TEXT    ( -- ) 27 EMIT [CHAR] [ EMIT ." 31m" ;
: GREEN-TEXT  ( -- ) 27 EMIT [CHAR] [ EMIT ." 32m" ;
: BLUE-TEXT   ( -- ) 27 EMIT [CHAR] [ EMIT ." 34m" ;
...
: ERROR1	\ ( C-ADDR U -- ) display an error message 
   ...
   TYPE 
   RED-TEXT SOURCE TYPE NORMAL-TEXT CR	\ display line corresponding to error
...
: T{		\ ( -- ) syntactic sugar.
   DEPTH START-DEPTH ! 0 XCURSOR ! F{ ; nondeferred
...
: COMMENT ( -- )
   BLUE-TEXT 10 WORD COUNT TYPE CR NORMAL-TEXT ;
