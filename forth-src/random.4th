...
\  2. https://www.sprng.org ; see Version 1.0: Users Guide: Generators
...
64-bit? [IF]
c" 28629335555777941757" number? drop 
                     2constant LCG_MUL   \ [2]
...
55                   constant  ROL9_RS
[ELSE]
1664525 s>d          2constant LCG_MUL   \ [3]
...
: random  ( -- u ) LCG_MUL seed @ udm* 2drop LCG_ADD + dup seed ! ;

: rol9 ( u1 -- u2 | rotate u1 left by 9 bits )
    dup ROL9_MASK and ROL9_RS rshift swap 9 lshift or ;     

: random2 ( -- u ) LCG_MUL seed @ udm* 2drop LCG_ADD + rol9 dup seed ! ;
